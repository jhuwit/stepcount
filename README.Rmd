...
See https://github.com/OxWearables/stepcount?tab=readme-ov-file#install for how to install the `stepcount` python module.  In the new `reticulate`, you can do this via:
...
sc <- import("stepcount")
```
This will install stepcount via `uv` **every time** you rhn the command a new time.
...
The `stepcount_check` function can determine if the `stepcount` module can be loaded and run:
